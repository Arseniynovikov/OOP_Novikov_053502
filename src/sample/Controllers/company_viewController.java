...
public class company_viewController {
    ...
    void initialize() {
        ...
        String current_unp = company.getUnp();
        String current_bik = Controller.bank.getBik();

        company.setAccounts(Take_Account.Accounts(current_unp, current_bik));
        company.setCredits(Credit_From_Sql.Take(current_unp, current_bik));
        company.setInstallment_plans(Installment_Plan_From_Sql.Take(current_unp , current_bik));

        final String[][] accounts_output = {new String[company.getAccounts().size()]};
        for(int i = 0; i < company.getAccounts().size(); i++) {
            accounts_output[0][i] = Make_account_string(company.getAccounts().get(i), i);
        ...
        make_accountButton.setOnAction(event -> {
            showComboBox.setValue("Счета");
            String account_number = company.getName() + Integer.toString(company.getAccounts().size());
            try {
                ...
                String buf_str = Make_account_string(bank_account, company.getAccounts().size());
        ...
        showComboBox.setOnAction(event -> {
            if(showComboBox.getValue().equals("Кредиты")) {
                ...
                final String[][] credits_output = {new String[company.getCredits().size()]};
                for(int i = 0; i < company.getCredits().size(); i++) {
                    credits_output[0][i] = Make_credit_string(company.getCredits().get(i), i);
                }
                ...
                showListView.getItems().clear();
                final String[][] _accounts_output = {new String[company.getAccounts().size()]};
                for(int i = 0; i < company.getAccounts().size(); i++) {
                    _accounts_output[0][i] = Make_account_string(company.getAccounts().get(i), i);
                }
                ...
                showListView.getItems().clear();
                final String[][] instalment_plans_output = {new String[company.getInstallment_plans().size()]};
                for (int i = 0; i < company.getInstallment_plans().size(); i++) {
                    instalment_plans_output[0][i] = Make_instalmentplan_string(company.getInstallment_plans().get(i), i);
                }
